...
class Dishonesty
{
    ...
    public function searchData(Request $request)
    {
        ...
        $dishonestyWhere=[
            ['responsibility_fulfillment_record.is_delete', '=', 0],
        ];
        if($user_id != ''){
            $dishonestyWhere[] = ['responsibility_fulfillment_record.user_id','=',$user_id];
        }

        $list = DishonestyModel::hasWhere('user',function($query) use ($where) {
            $query->where($where);
        })
    ...
    public function updateData(Request $request)
    {
        ...
        $dishonestyModel = new DishonestyModel();
        $dishonestyModel->save($params, ['id' => $params['id']]);

        $uid = $this->loginTokenService->getCurrentUid();
        $sysUser = SysUserModel::get($uid);
        Hook::listen('logger', $sysUser['name'] . '更新了' . $user['name'].'的失信记录，记录id:['.$params['id'].']');
    ...
    {
        $params = $request->get();
        $dishonesty = DishonestyModel::get($params['id']);
        ...
        $uid = $this->loginTokenService->getCurrentUid();
        $sysUser = SysUserModel::get($uid);
        Hook::listen('logger', $sysUser['name'] . '删除了' . $user['name'].'的失信记录，记录id:['.$params['id'].']');
